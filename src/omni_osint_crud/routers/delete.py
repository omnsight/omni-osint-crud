...
def delete_entity(id: str, user_ctx: Dict = Depends(get_user_context)):
    if not dal.is_owner(id, user_ctx["user_id"]):
        raise HTTPException(
            status_code=403, detail="Only the owner can delete this resource"
        )
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="Entity not found or could not be deleted"
            )
...
def delete_relation(id: str, user_ctx: Dict = Depends(get_user_context)):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="Relation not found or could not be deleted"
            )
        return {"status": "success", "id": id}
    except Exception:
