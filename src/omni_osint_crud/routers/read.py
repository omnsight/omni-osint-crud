...
@router.get("/view/{id:path}/entities", response_model=List[Union[Relation, Event, Source, Person, Organization, Website]])
def get_view_entities(id: str, user_ctx: Dict = Depends(get_user_context)):
    try:
        ...
        logger.exception(f"User {user_ctx['user_id']} failed to read entities for view {id} due to insufficient permissions")
        raise HTTPException(status_code=403, detail="Insufficient permissions to access this resource")
    except Exception:
...
@router.get("/view/{id:path}", response_model=OsintView)
...
@router.get("/views", response_model=List[OsintView])
