...
class TestPerson:
    ...
    def test_person_crud_cycle(self):
        ...
        response = self.client.put(f"/persons/{person_id}", json=update_data.model_dump(exclude_unset=True))
        assert response.status_code == 200
        updated_person = response.json()
        assert updated_person["name"] == "John Doe Updated"

        # 4. Delete Person
        response = self.client.delete(f"/entities/{person_id}")
        assert response.status_code == 200
    ...
    def test_read_person_not_found(self):
        response = self.client.get("/persons/person/non-existent-id")
        assert response.status_code == 404

    def test_read_person_not_found_bad_id(self):
        response = self.client.get("/persons/bad_collection/bad_key")
        assert response.status_code == 404
    ...
    def test_read_person_permission_denied(self):
        ...
        response = self.no_roles_client.get(f"/persons/{person_id}")
        assert response.status_code == 403
    ...
    def test_read_person_internal_error(self, mock_read_person):
        ...
        response = self.client.get("/persons/person/some-id")
        assert response.status_code == 500
    ...
    def test_update_person_not_found(self):
        ...
        response = self.client.put("/persons/person/non-existent-id", json=update_data.model_dump(exclude_unset=True))
        assert response.status_code == 404
    ...
    def test_update_person_permission_denied(self):
        ...
        update_data = PersonMainData(name="Jane Doe")
        response = self.no_roles_client.put(f"/persons/{person_id}", json=update_data.model_dump(exclude_unset=True))
        assert response.status_code == 403
    ...
    def test_update_person_internal_error(self, mock_update_person):
        ...
        update_data = PersonMainData(name="Jane Doe")
        response = self.client.put("/persons/person/some-id", json=update_data.model_dump(exclude_unset=True))
        assert response.status_code == 500
    ...
    def test_update_person_permissions_not_found(self):
        ...
        response = self.client.put("/persons/person/non-existent-id/permissions", json=update_data)
        assert response.status_code == 404
    ...
    def test_update_person_permissions_permission_denied(self):
        ...
        response = self.client.put(f"/persons/{person_id}/permissions", json=permission_data)
        assert response.status_code == 200

        # 3. Attempt to update permissions with the user_client (non-owner)
        update_data = {"owner": "new-owner"}
        response = self.user_client.put(f"/persons/{person_id}/permissions", json=update_data)
        assert response.status_code == 403
    ...
    def test_update_person_permissions_internal_error(self, mock_update_person):
        mock_update_person.side_effect = Exception("DB error")
        update_data = {"owner": "new-owner"}
        response = self.client.put("/persons/persons/some-id/permissions", json=update_data)
        assert response.status_code == 500
    ...
    def test_delete_person_not_found(self):
        response = self.client.delete("/entities/persons/non-existent-id")
        assert response.status_code == 404
    ...
    def test_delete_person_permission_denied(self):
        ...
        response = self.no_roles_client.delete(f"/entities/{person_id}")
        assert response.status_code == 403
    ...
    def test_delete_person_internal_error(self, mock_delete_entity):
        ...
        response = self.client.delete("/entities/persons/some-id")
        assert response.status_code == 500
