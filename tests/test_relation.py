...
class TestRelation:
    ...
    def test_relation_crud_cycle(self):
        ...
        person_res = self.client.post("/persons", json=person_data.model_dump(exclude_unset=True))
        assert person_res.status_code == 200
        person = person_res.json()
        person_id = person["_id"]

        org_data = OrganizationMainData(name="Test Org for Relation")
        org_res = self.client.post("/organizations", json=org_data.model_dump(exclude_unset=True))
        ...
        response = self.client.post("/relations", json=create_data.model_dump(exclude_unset=True))
    ...
    def test_create_relation_permission_denied(self):
        ...
        response = self.no_roles_client.post("/relations", json=create_data.model_dump(exclude_unset=True))
