...
class TestView:
    ...
    def test_view_crud_cycle(self):
        ...
        response = self.client.put(f"/view/{view_id}", json=update_data.model_dump(exclude_unset=True))
        assert response.status_code == 200
    ...
    def test_read_view_not_found(self):
        response = self.client.get("/view/bad_collection/bad_key")
        assert response.status_code == 404
    ...
    def test_get_view_entities_internal_error(self, mock_get_entities):
        ...
        response = self.client.get("/view/bad-collection/bad-key/entities")
        assert response.status_code == 500
    ...
    def test_update_view_not_found(self):
        update_data = OsintViewMainData(name="Test View Updated")
        response = self.client.put("/view/bad_collection/bad_key", json=update_data.model_dump(exclude_unset=True))
        assert response.status_code == 404
    ...
    def test_update_view_permission_denied(self):
        ...
        update_data = OsintViewMainData(name="Test View Updated")
        response = self.no_roles_client.put(f"/view/{view_id}", json=update_data.model_dump(exclude_unset=True))
        assert response.status_code == 403
    ...
    def test_update_view_permissions_permission_denied(self):
        ...
        response = self.client.put(f"/view/{view_id}/permissions", json=permission_data)
        assert response.status_code == 200

        # 3. Attempt to update permissions with the user_client (non-owner)
        update_data = {"owner": "new-owner"}
        response = self.user_client.put(f"/view/{view_id}/permissions", json=update_data)
        assert response.status_code == 403
    ...
    def test_connect_entity_to_view_permission_denied(self):
        ...
        response = self.no_roles_client.post(f"/view/{view_id}/entities", json={"entity_id": person_id})
        assert response.status_code == 403
    ...
    def test_delete_view_not_found(self):
        response = self.client.delete("/view/bad_collection/bad_key")
        assert response.status_code == 404
    ...
    def test_delete_view_permission_denied(self):
        ...
        response = self.no_roles_client.delete(f"/view/{view_id}")
        assert response.status_code == 403
