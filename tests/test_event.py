...
class TestEvent:
    ...
    def test_read_event_not_found(self):
        response = self.client.get("/event/non-existent-id")
        assert response.status_code == 404
    ...
    def test_read_event_internal_error(self, mock_read_event):
        ...
        response = self.client.get("/event/some-id")
        assert response.status_code == 500
    ...
    def test_update_event_not_found(self):
        ...
        response = self.client.put("/event/non-existent-id", json=update_data.model_dump(exclude_unset=True))
        assert response.status_code == 404
    ...
    def test_update_event_internal_error(self, mock_update_event):
        ...
        update_data = EventMainData(title="Jane Doe")
        response = self.client.put("/event/some-id", json=update_data.model_dump(exclude_unset=True))
        assert response.status_code == 500
    ...
    def test_update_event_permissions_not_found(self):
        ...
        response = self.client.put("/event/non-existent-id/permissions", json=update_data)
        assert response.status_code == 404
    ...
    def test_update_event_permissions_internal_error(self, mock_update_event):
        mock_update_event.side_effect = Exception("DB error")
        update_data = {"owner": "new-owner"}
        response = self.client.put("/event/some-id/permissions", json=update_data)
        assert response.status_code == 500
